...
public class AppletGUI extends JApplet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7154491956009019731L;
	...
	private void initialize() {
		...
		conDB.setModel(
				new DefaultComboBoxModel<Object>(new String[] { "--Please Select Database--", "MYSQL", "ORACLE" }));
		...
		mntmAbout.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showMessageDialog(dialog, "Query By Example \n \u00a9 spectral369 \n" + "Version 1.0");
			}
		});
	...
	protected void restartActionPerformed(ActionEvent e) {
		// TODO Auto-generated method stub
		getAppletContext().showDocument(this.getDocumentBase());
	...
	protected void connectionButtonActionPerformed(ActionEvent e) {
		...
		if (checkInfo) {
			...
			if (conDB.getSelectedItem().equals("MYSQL")) {
				api.setMYSQLServerInformation(username.getText(), String.copyValueOf(password.getPassword()),
				...
				if (connectionValid) {
					...
					try {
						...
						JOptionPane.showConfirmDialog(frame, "NO databases available \n for current user");
					}
				...
				api.setOracleServerInformation(username.getText(), String.copyValueOf(password.getPassword()),
						server.getText(), Integer.parseInt(port.getText()), oracleSID.getText());
				api.beginSession();
				try {
	...
	protected void QueryActionPerformed(ActionEvent e) {
		...
		if (api.getDatabaseAvailable().toString().equals("MYSQL")) {
			...
			} else if (collsSelected == false && columnLIst.getModel().getSize() > 1) {
			...
			queryData = api.SQLQBE(schemas.getSelectedItem().toString(), tableList.getSelectedValue().toString(),
					queryTextField.getText(), sel);
			...
			queryData = new QueryData();
			queryData = api.OracleQBE(schemas.getSelectedItem().toString(), tableList.getSelectedValue().toString(),
					queryTextField.getText(), sel);
		...
		if (columnLIst.isSelectionEmpty()) {
			if (columnLIst.getSelectedValuesList().isEmpty()) {
				...
				if (columnLIst.getSelectedValuesList().size() > 0) {
					ColsSelected = new String[columnLIst.getSelectedValuesList().size()];
